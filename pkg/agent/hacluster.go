...
func (hac *HACluster) GetSchema(dbfilter string) ([]*InfluxSchDb, error) {
	...
	for _, db := range srcDBs {
		...
		//
		schema = append(schema, &InfluxSchDb{Name: db, Newname: db, DefRp: defaultRp.Name, Rps: rps, Ftypes: mf})
	}
	hac.Schema = schema
	return schema, nil
}
...
func (hac *HACluster) ReplicateData(schema []*InfluxSchDb, start time.Time, end time.Time) error {
	for _, db := range schema {
		for _, rp := range db.Rps {
			...
			err := SyncDBRP(hac.Master, hac.Slave, db.Name, db.Newname, rp, start, end, db, hac.ChunkDuration, hac.MaxRetentionInterval)
			if err != nil {
